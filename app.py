...
def load_model():
    try:
        ...
        print(
            "CRITICAL: model.pkl or model_columns.pkl not found. Please train the model first."
        )
        return None, None
...
def recommend():
    if model is None:
        flash(
            "Model not loaded. Please ensure 'model.pkl' and 'model_columns.pkl' exist.",
            "danger",
        )
        return redirect(url_for("dashboard"))
    ...
    try:
        ...
        flash(
            "Please enter valid numbers for Weight, Temperature, and Sleep Hours.",
            "danger",
        )
